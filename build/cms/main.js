...
cms.post('/setUserRole', function (req, res) {
  ...
  if (username) {
    if (!authorityCheck(req, username)){
      result.code = -100
      result.message = '权限不足'
      res.send(result)
      return
...
cms.post('/saveMenu', function (req, res) {
  ...
  if (username) {
    ...
    if (user) {
      ...
      o.authority = req.body.authority.toString()
...
cms.post('/saveShop', function (req, res) {
  ...
  if (username) {
    ...
    if (user) {
      ...
      o.status = (req.body.status)
      o.phone = (req.body.phone.toString())
      if (req.body.id) {
...
cms.post('/saveGood', function (req, res) {
  ...
  if (username) {
    ...
    if (user) {
      ...
      o.status = req.body.status
      if (req.body.id) {
        var shop = memory.shops.searchByKey("id", req.body.shopId)
        if (shop) {
          o.id = (req.body.id)
          _sqlclass.modifyGood(o, function (status) {
            if (status) {
              result.code = 0
              result.message = 'Save success'
              res.send(result)
...
cms.post('/saveRoleResources', function (req, res) {
  ...
  if (username) {
    ...
    if (user) {
      ...
      o.authoritys = JSON.stringify(req.body.authoritys)
...
var authorityCheck = (req,username) => {
  return true;
  var user = memory.users.searchByKey('username', username)
  if(!user)return false
  var userResources = memory.rolesResources.searchByKey('roleId', user.roleId)
  var userAuthority = []
  var url = req.originalUrl
  userResources.menuIds.forEach(function(id) {
    memory.menus.forEach(function(menu) {
      if(menu.id === id)
      {
        menu.authority.forEach(function(a) {
          userAuthority.push(a)
        });
      }
    });
  });
  console.log(userAuthority)
  if(userAuthority.indexOf(url))
  {
    return true
  }else {
    return false
